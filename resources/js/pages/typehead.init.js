...
$(document).ready(function () {
    ...
    function loadCompanies() {
        $('.autocomplete').each(function() {
            var companyIdInput = $(this).attr('id').replace('companies_', '#company_id_');
            var fieldName = $(this).attr('data-field-name');

            if (!companiesData[fieldName]) {
                // Cargar data de companies solo si no está cargada previamente
                var companies = new Bloodhound({
                    datumTokenizer: Bloodhound.tokenizers.obj.whitespace('Nombre'),
                    queryTokenizer: Bloodhound.tokenizers.whitespace,
                    prefetch: {
                        url: '/report/search/company',
                        ttl: 0,
                ...
                companiesData[fieldName] = companies;
            }

            // Reemplazar la clase autocomplete por la clase companies
            $(this).removeClass('autocomplete').addClass('companies');
            ...
            $(this).typeahead({
                ...
                name: 'tercero',
                display: 'Nombre',
                source: companiesData[fieldName]
            }).on('typeahead:select', function(event, suggestion) {
                $(companyIdInput).val(suggestion.id);
            });
        });
    }

    // Cargar companies al inicio
    /*loadCompanies();*/
});
